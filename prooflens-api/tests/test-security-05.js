 ...
 *   F. Custom rate-limit enforcement
 *
...
console.log("\nSECTION F - Custom rate-limit enforcement\n");

check("RATE-LIMIT-FAILS-CLOSED-AFTER-BOUND", () => {
  const { rateLimit } = require(path.join(__dirname, "..", "dist", "utils", "rateLimit.js"));
  const limiter = rateLimit({
    windowMs: 60_000,
    maxPerIp: 2,
    keyPrefix: `security-test-${Date.now()}`,
  });
  const req = {
    ip: "203.0.113.77",
  ...
  assert.strictEqual(allowed, 2, "only requests inside the configured bound may continue");
  assert.strictEqual(res.statusCode, 429);
  assert.deepStrictEqual(res.body, { error: "rate_limit_exceeded" });
