...
const TSA_REQUEST_TIMEOUT_MS = 15_000;
...
export async function requestTsa(tsqDer: Buffer): Promise<Buffer> {
  ...
  if (!contentType.includes("application/timestamp-reply")) {
    throw new Error(`[tsa] Unexpected response content type: ${contentType || "missing"}`);
  }

  const declaredLength = Number(res.headers.get("content-length"));
  if (Number.isFinite(declaredLength) && declaredLength > MAX_TSA_RESPONSE_BYTES) {
    throw new Error("[tsa] TSA response exceeds size limit");
  }

  const response = Buffer.from(await res.arrayBuffer());
  if (response.length === 0 || response.length > MAX_TSA_RESPONSE_BYTES) {
    throw new Error("[tsa] TSA response is empty or exceeds size limit");
  }
  return response;
}
